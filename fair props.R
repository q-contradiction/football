...
# method jsd is not supported with 'implied_odds', additive same transformation as wpo, exclude both
...
props_table <- expand.grid(bookmaker = c("Pinnacle", "Bet365", "Bwin", "WilliamHill"),
                           method = methods[1:6], 
                           stringsAsFactors = F) %>%
  pmap_dfr(fair_probabilities, market = "1X2", .progress = T)

## rank probability score ----
cum_props <- t(apply(data.matrix(select(props_table, PH, PD, PA)), 1, cumsum))
...
res_props <- left_join(props_table, res, by = "Res") %>%
  select(r1, r2, r3) %>% data.matrix()

props_table$RPS <- 0.5*rowSums((cum_props - res_props)^2)

rank_probability_score <-  props_table %>%
...
fair_bets <- fair_returns(props_table) 
...
fair_bets %>% 
  ...
  theme_bw() +
  labs(title = "Profit n Loss with fair odds from different bookies",
       subtitle = "Different ovverround removal methods")
